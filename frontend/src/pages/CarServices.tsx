...
import { Wrench, Droplets, Settings, Gauge, Paintbrush, Shield, Car, type LucideIcon } from "lucide-react";
import { listServices, getAuth } from "@/lib/utils";

const fadeUp = { initial: { opacity: 0, y: 30 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true }, transition: { duration: 0.5 } };

const iconMap: Record<string, LucideIcon> = { General: Wrench, Oil: Droplets, Engine: Settings, Performance: Gauge, Detailing: Paintbrush, Insurance: Shield };

const CarServices = () => {
  const navigate = useNavigate();
  const carServices = listServices("car").map((s) => ({
          ...
          {carServices.map((s, i) => (
            ...
            >
              ...
              <button
                ...
                onClick={() => {
                  const target = `/book/date?vehicle=car&service=${encodeURIComponent(s.title)}&price=${encodeURIComponent(s.price)}`;
